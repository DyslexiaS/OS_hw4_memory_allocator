...
void split_chunk(struct chunk_header* org_chunk, size_t bytes) //TODO smaller than 48
{
    ...
    if(rest_chunk->chunk_size == HEAP_SIZE){
        ...
        init_chunk_header(rest_chunk,org_chunk->chunk_size-bytes,bytes,0);
        init_chunk_header(org_chunk,bytes,org_chunk->prev_chunk_size,org_chunk->prev_free_flag);
    }
}
